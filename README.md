...
| 知识库 | 项目内▸知识库 | 项目文档库(需求/资料随取随用) |
| AI 生成任务 | 项目内▸AI测试▸生成任务 | PRD/文档 → AI 生成功能用例 → 暂存区裁决入树 |
| 自动化工程 | 项目内▸AI测试▸自动化工程 | AI 生成接口自动化脚本 + Git 仓库推送 |
| 多端 UI 自动化 | 项目内▸AI测试▸多端 UI 自动化 | 实验性(Web/Android/鸿蒙 AI 执行闭环) |
| Web 自动化 | 项目内▸UI自动化▸Web自动化 | Playwright 录制 → DSL → 回放执行,可导出 pytest |
| APP 自动化 | 项目内▸UI自动化▸APP自动化 | SoloPi 采集/回放(Android) |
| HTTP Mock | 项目内▸接口Mock▸HTTP Mock | 每项目多实例 mock server,规则匹配 + 模板响应 |
| APP 性能测试 | 项目内▸性能测试▸APP性能测试 | 性能记录/详情/跨记录对比/趋势/设备历史导入 |
...
1. **新建实例**:命名 + 指定端口(留空自动分配),可选开启 CORS;拿到 `http://localhost:<端口>` 即可指向被测系统;
2. **配规则**:
   - 路径模板只填**纯路径**(如 `/v1/forecast`,动态段用 `{var}`);
   - 匹配条件三作用域:**查询参数 / 请求头 / 请求体**——请求体的参数名填 **JSONPath**(如 `$.user.id`,
     支持数组下标 `$.data.list[0].id`),匹配方式 eq(相等)/ regex(正则,如 `^admin` 即前缀匹配);
   - 多条件 AND 组合,按排序取第一条命中的规则;
   - 响应体支持 **Jinja2 模板**(`{{ path.id }}`、`{{ uuid4() }}` 等),可配延迟(ms)与模拟超时;
3. **看命中**:每次请求落命中记录(请求/响应原文,1000 条滚动),mock 未接住的请求回实例默认响应。

上例:实例「天气查询」(9001) 配规则 `GET /v1/forecast` + 条件 `$.latitude` 等,即可对天气接口做确定性模拟。

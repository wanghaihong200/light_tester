...
- **匹配**:两级遍历取**第一条命中**——先按**组间序**遍历规则组,再按**组内序**遍历组内规则;组=HTTP 方法精确 + 路径精确或 `{var}` 模板段(同实例内唯一,重路 400),规则=条件行(query/header/body 按 JSONPath,等于|正则)+响应;组或规则停用即跳过(组停用整组跳过);全不中:实例开了透传→原样转发上游(见「透传」),否则回实例默认响应(`default_status`/`default_body`)。存量迁移保序:组序=旧全局扁平序的最小 `sort_order`,迁移前后行为等价(ADR-0011)。
